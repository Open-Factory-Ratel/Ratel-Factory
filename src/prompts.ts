...
After user approval, run workers serially for each feature in the current milestone. After all features complete:
   a. Before calling run_worker, consider calling get_feature_complexity. Features with many assertions or spanning multiple .feature files may be too large for a single worker and could timeout. If a feature is large, consider splitting it into smaller sub-features before spawning.
   b. Call run_worker() for each feature. **Check \x60details.parseStatus\x60 in the tool result.** If it is "failed", halt immediately. Do not infer success — the worker did not produce a parseable handoff.

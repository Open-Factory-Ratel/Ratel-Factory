...
After user approval, run workers serially for each feature in the current milestone. After all features complete:
   a. Call run_worker() for each feature. **Check \x60details.parseStatus\x60 in the tool result.** If it is "failed", halt immediately. Do not infer success — the worker did not produce a parseable handoff.
   b. If the worker's parseStatus is "ok", inspect the handoff fields: \x60issuesDiscovered\x60, \x60leftUndone\x60, \x60proceduresAbided\x60, \x60summary\x60, and \x60details.workspaceFinalization\x60. Do NOT mark the feature complete unless the handoff is clean AND workspaceFinalization is "merged" (or "skipped" for missions with no integration repo). If finalization is "blocked" or "no_changes", halt or create recovery work; do not proceed to validation with unmerged work. Call \x60mark_feature_completed(featureId)\x60 to request the completion transition. The gate validates integrity and either applies it or explains why it was rejected. Do NOT write \x60features.json\x60 directly to mark a feature completed.
   c. After all features in the milestone complete, call run_validation() to trigger the Scrutiny Validator.
   d. **First check \x60details.preflightStatus\x60.** If it is "failed", validators did NOT run because completed feature commits are missing from the canonical integration branch. This is MERGE RECOVERY MODE — NOT A HALT. Create a same-milestone merge recovery feature using \x60details.recovery\x60 / \x60details.preflight\x60, run the worker, verify integration contains the missing commits/equivalent diffs, then call run_validation() again.
   e. **Then check \x60details.parseStatus\x60 in the scrutiny result.** If it is "failed", halt immediately (call \x60halt_mission\x60 with the raw text in context). Do not proceed to user testing.
   f. If parseStatus is "ok", read the report from \x60details.report\x60 and inspect \x60issues[]\x60 for blocking issues. If any \x60severity === "blocking"\x60, this is VALIDATION RECOVERY MODE — NOT A HALT. Create fix features IN THE SAME MILESTONE, preferably using \x60details.recovery.suggestedFixFeatures\x60 as the starting point.
   g. If scrutiny passes (no blocking issues), call run_user_testing().
   h. **First check \x60details.preflightStatus\x60 in the user-testing result.** If it is "failed", create a same-milestone merge recovery feature; do not halt solely for this.
   i. **Then check \x60details.parseStatus\x60 in the user testing result.** If "failed", halt.
   j. If user testing parseStatus is "ok", inspect its \x60issues[]\x60 for blocking issues. If any, this is VALIDATION RECOVERY MODE — NOT A HALT. Create fix features IN THE SAME MILESTONE, preferably using \x60details.recovery.suggestedFixFeatures\x60 as the starting point.
   k. Repeat until both validators pass. Halt only if validation is not converging after 5 rounds or recovery requires user input.
